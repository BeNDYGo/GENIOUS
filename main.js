...
const SampleButton = '<button id="ButtonSearhGenious" style="background: none; border: none; color: #808080; font-size: 18px; font-weight: 600; padding: 8px 12px; cursor: pointer; transition: color 0.3s ease, transform 0.2s ease; position: relative; z-index: 1000;">G</button>'
let TrackName = '';
let TreckAutors = [];
...
function Main(){
    ...
    if (BarMeta && !document.getElementById('ButtonSearhGenious')) {
        ...
        document.getElementById('ButtonSearhGenious').addEventListener('click', (e) => {
            e.stopPropagation();
...
const BarPalyer = document.querySelector('[class^="PlayerBarDesktopWithBackgroundProgressBar_description"]')
if (BarPalyer) BarPalyer.addEventListener('click', Main);
Main({ target: {} });